...
for track_name in setlist:
    ...
    if  SAVE_RENDER:
        ...
        ani.save(spectrogram_output_filename, fps=fps)
        ...
        subprocess.run(['ffmpeg', '-i', spectrogram_output_filename, '-i', audio_output_filename, '-c:v', 'copy', '-c:a', 'aac', spectrogram_with_audio_output_filename, '-strict', '-2'])

        # Delete temp video and audio track files
        print('Cleaning up temp files ...')
        os.remove(audio_output_filename)
        # os.remove(spectrogram_output_filename)

        # Open rendered video in default system app
        # subprocess.run(('xdg-open', spectrogram_with_audio_output_filename))
